...
ps <- readRDS("C:/Users/cfadhelalvarez/Desktop/perez_diet/rds_objects/PerezDiet_merged_Len250_EE5_5_RefSeq.rds")
```

Now, we extract taxonomy and build a table with the sequences to blast and empty columns to input the blast results.
```{r}
...
write.csv(blast_table, "C:/Users/cfadhelalvarez/Desktop/perez_diet/tables/empty_BLAST_table_PerezDiet_merged_Len250_EE5_5.csv", row.names = FALSE)
```

Manually add BLAST results or do it outside of R and read in table (if so, skip this step).
```{r}
# Manually add BLAST results or do it outside of R and read in table (if so, skip this step).
...
# Save final version
write.csv(blast_table, "C:/Users/cfadhelalvarez/Desktop/perez_diet/tables/BLAST_table_PerezDiet_merged_Len250_EE5_5.csv", row.names = FALSE)
```

If starting from the BLAST table: Read in the Excel files (Sheet 2 has names only without match percentages)
```{r}
# Read in tables
forward_df <- read_excel("C:/Users/cfadhelalvarez/Desktop/perez_diet/tables/BLAST_table_PerezDiet_forward_Len250_f5_rNULL.xlsx", sheet = 2)
merged_df <- read_excel("C:/Users/cfadhelalvarez/Desktop/perez_diet/tables/BLAST_table_PerezDiet_merged_Len250_EE5_5.xlsx", sheet = 2)
...
Now you can create the confusion matrix showing the presense and absense of genera.
```{r}
...
You can visualize this by creating a Venn Diagram.
```{r}
# Create Venn Diagram
...
# Step 7: Optional – Export tables (if not exported already)
write.csv(confusion_matrix, "C:/Users/cfadhelalvarez/Desktop/perez_diet/tables/genus_confusion_matrix_blast_only.csv", row.names = FALSE)
write.csv(otu_trace_matrix, "C:/Users/cfadhelalvarez/Desktop/perez_diet/tables/genus_to_otus_trace_blast_only.csv", row.names = FALSE)
```